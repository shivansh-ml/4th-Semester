...
# Access Items: You can access the list items by referring to the index
# number. Python uses zero-based indexing.
print(my_list[0]) # Outputs: 1 Positive indexing
print(my_list[-1]) # Outputs: banana (negative indexing starts from the end)
...
print(my_list) # Outputs: [1, 'mango', 8, 9, 'banana']

# • Add Items: You can use the append() method to add an item at the end
# of the list. Use the insert() method to add an item at a specific position.
...
# • Remove Items: The remove() method removes the specified item. The
# pop() method removes the specified index. If you do not specify the
...
# • Sort List: The sort() method sorts the list ascending by default. You
# can also make it descending by making reverse=True.
my_list.sort()
print(my_list) # Outputs: [1, 3, 'apple', 'banana', 'cherry']
...
# • Extends List: You can extend a list by appending elements from another
